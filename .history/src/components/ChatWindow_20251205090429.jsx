    ...
    return (
        ...
        >
            ...
            <div className={`flex flex-col min-w-0 flex-1 max-w-[85%] md:max-w-[70%] ${isOutgoing ? 'items-end' : 'items-start'}`}>
                <span className="text-xs font-semibold mb-1 px-1 text-slate-100">{displayName}</span>
                ...
                >
                    {isEditing ? (
                        ...
                        <>
                            ...
                            <span className="absolute bottom-1 right-2 text-[10px] text-slate-500">{timestamp}</span>

                            {/* Actions for outgoing messages */}
                            {isOutgoing && (
                                <div
...
function ChatWindow({
    ...
    const saveEdit = useCallback((messageId) => {
        if (editContent.trim()) {
    ...
    const toggleActions = useCallback((messageId) => {
