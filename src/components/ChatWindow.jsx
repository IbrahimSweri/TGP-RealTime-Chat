import { useState, useRef, memo, useEffect } from 'react' // Import memo
        ...
        <div className={`flex gap-2 px-2 py-2 group ${isOutgoing ? 'flex-row-reverse' : 'flex-row'}`}>
            ...
            <div className={`flex flex-col min-w-0 flex-1 max-w-[85%] md:max-w-[70%] ${isOutgoing ? 'items-end' : 'items-start'}`}>
                ...
                <div
                    ...
                    role={isOutgoing && !isEditing ? 'button' : undefined}
                    tabIndex={isOutgoing && !isEditing ? 0 : undefined}
                    onKeyDown={(e) => {
                        if (isOutgoing && !isEditing && (e.key === 'Enter' || e.key === ' ')) {
                            e.preventDefault()
                            toggleActions(message.id)
                        }
                    }}
                    aria-label={isOutgoing && !isEditing ? 'Message actions' : undefined}
                >
                    {isEditing ? (
                                ...
                                >
                                    <div className="flex gap-1 bg-slate-800/90 backdrop-blur rounded-lg p-1 border border-white/10 shadow-lg">
                                        <button
                                            ...
                                            aria-label="Edit message"
                                        >
                                            <svg xmlns="http://www.w3.org/2000/svg" width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" aria-hidden="true"><path d="M17 3a2.828 2.828 0 1 1 4 4L7.5 20.5 2 22l1.5-5.5L17 3z"></path></svg>
                                        </button>
                                        <button
                                            onClick={(e) => { e.stopPropagation(); setDeleteConfirm({ isOpen: true, messageId: message.id }); setShowActionsForMessage(null) }}
